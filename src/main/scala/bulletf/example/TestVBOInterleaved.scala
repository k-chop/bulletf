...
object TestVBOInterleaved {
  ...
  def setupQuad() {
    ...
    val v0 = Vertex.xyz(-0.5f, 0.5f, 0f).rgb(1, 0, 0).x
    val v1 = Vertex.xyz(-0.5f, -0.5f, 0f).rgb(0, 1, 0).x
    val v2 = Vertex.xyz(0.5f, -0.5f, 0f).rgb(0, 0, 1).x
    val v3 = Vertex.xyz(0.5f, 0.5f, 0f).rgb(1, 1, 1).x

    val vertices = Array[Vertex](v0, v1, v2, v3)
    val verticesBuffer = BufferUtils.createFloatBuffer(vertices.length * Vertex.elementCount)
    vertices foreach { v =>
